...
from widerface_evaluate.evaluation import get_val_preds, evaluation
...
def train_model(
    ...
    # Data augmentation
    train_augmenter = MyPreproc(target_size, kpt_flip_map=[(0, 1), (3, 4)])
    
    # Dataset and dataloader
    dataset = WiderFaceDataset(
        root=train_root, 
        label_file=train_label_file,
        transforms=train_augmenter
    )
    ...
    for epoch_num in range(start_epoch, num_epochs):
        ...
        for iter_num, (imgs, boxes_gt, kpts, kpts_vis) in enumerate(dataloader):
            ...
            # Total loss
            loss = total_obj + total_iou + total_bbox + 0.01 * total_kpt
        ...
        save_checkpoint(model, optimizer, scheduler, epoch_num, epoch_total_loss, f'{checkpoint_dir}/model_epoch_{epoch_num}.pth')
        
        # Evaluate on validation set periodically
        if (epoch_num % eval_frequency == 0 and epoch_num >= -2):
